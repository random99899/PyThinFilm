...
2. half_wave_single_layer: optical phase thickness 180 deg.
3. high_reflector: R_max > 0.95 & Bragg stopband segment.
4. quarter_wave_stack: periods = 3.5 & TE/TM stopbands.
5. fp_single_halfwave: intra-stopband defect peak.
6. narrowband_filter: 17 layers (periods=5 -> (HL)^4 C (LH)^4) & intra-stopband peak.
"""
...
def test_half_wave_single_layer_metrics():
    ...
    metrics = data["case_specific_metrics"]
    assert metrics["optical_phase_thickness_at_design_deg"] == 180.0
    assert metrics["difference_from_bare_substrate_at_0deg"] == 0.0
