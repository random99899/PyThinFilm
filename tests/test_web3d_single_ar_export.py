# -*- coding: utf-8 -*-
"""Pytest verification for single_ar exported JSON data (Stage B.1A.1).

Validates:
1. Re-invokes Python TMM core simulate_report_design('single_ar').
2. Checks web3d/public/results/single_ar.json against direct Python calculation.
...
6. First point, 550nm design point, and last point value matching.
"""
...
def test_single_ar_python_export_verification():
    json_path = ROOT / "web3d" / "public" / "results" / "single_ar.json"
    assert json_path.exists(), f"single_ar.json file missing at {json_path}"
    ...
    # 5. First point, 550nm design point, and last point verification
    idx_550 = int(np.argmin(np.abs(wl_py - 550.0)))
    assert wl_json[idx_550] == 550.0
    assert abs(data["design_point_550nm"]["TE"]["R"] - res_te["R"][idx_550]) < 1e-5
    assert abs(data["design_point_550nm"]["TM"]["R"] - res_tm["R"][idx_550]) < 1e-5
