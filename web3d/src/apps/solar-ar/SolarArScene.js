...
import { sampleSolarSpectrum } from "./solarSpectrum.js";

export const SOLAR_WAVE_DISPLAY_SCALE = 2.2;

export class SolarArScene {
  constructor({ container, camera, domElement, onLayerFocus, onWaveInfo }) {
    ...
    this.group.name = "StandaloneSolarArScene";
  ...
  build(caseResult, options = {}) {
    ...
    if (layers.length !== 3) throw new Error(`solar 独立 App 要求 3 层正式膜层，实际为 ${layers.length} 层`);
    ...
    const substrateName = caseResult.substrate?.name || "Si";
    ...
    substrate.name = "solar_substrate";
    substrate.position.y = currentY - THICKNESS_MAPPING.substrateVisual / 2;
    substrate.userData = { materialName: substrateName, isSubstrate: true };
  ...
  rebuildWaves() {
    this.waveAmpInfo = sampleSolarSpectrum(this.caseResult, this.polarization, this.selectedWavelengthNm);
    ...
    const descriptors = [
      {
        id: "incident_wave",
        start: [-4 * Math.sin(angleRad), 4 * Math.cos(angleRad), 0],
        end: origin,
        amplitude: 0.25,
        wavelength: 0.8,
        speed: 2,
        pol: this.polarization,
        color: theme.waveColors.incident,
        linewidth: 1,
        opacity: 0.92,
        displayScale: SOLAR_WAVE_DISPLAY_SCALE,
      },
      {
        id: "reflected_wave",
        start: origin,
        end: [4 * Math.sin(angleRad), 4 * Math.cos(angleRad), 0],
        amplitude: this.waveAmpInfo.rAmp,
        wavelength: 0.8,
        speed: 2,
        pol: this.polarization,
        color: theme.waveColors.reflected,
        linewidth: 1,
        opacity: 0.92,
        displayScale: SOLAR_WAVE_DISPLAY_SCALE,
      },
      {
        id: "transmitted_wave",
        start: origin,
        end: [1.5 * Math.sin(angleRad * 0.7), -4 * Math.cos(angleRad * 0.7), 0],
        amplitude: this.waveAmpInfo.tAmp,
        wavelength: 0.6,
        speed: 2,
        pol: this.polarization,
        color: theme.waveColors.transmitted,
        linewidth: 1,
        opacity: 0.92,
        displayScale: SOLAR_WAVE_DISPLAY_SCALE,
      },
  ...
  rebuildDirectionArrows(descriptors) {
    ...
    this.directionGroup.name = "solar_wave_directions";
    descriptors.forEach((descriptor) => {
      ...
      const arrow = new THREE.ArrowHelper(
        direction,
        start.clone().addScaledVector(direction, length * 0.62),
        0.52,
        descriptor.color,
        0.15,
        0.08
      );
      this.directionGroup.add(arrow);
    });
  ...
  rebuildPolarizationArrow() {
    ...
    const theme = getVisualTheme("ACADEMIC_LIGHT");
    const direction = this.polarization === "TE"
      ? new THREE.Vector3(0, 0, 1)
      : new THREE.Vector3(1, 0, 0);
    const color = this.polarization === "TE"
    ...
    this.polarizationArrow.name = "solar_polarization_arrow";
  ...
  setHoveredLayer(index) {
    this.hoveredLayerIndex = Number.isInteger(index) ? index : null;
    this.refreshLayerState();
  }

  setSelectedLayer(index) {
    this.selectedLayerIndex = Number.isInteger(index) ? index : null;
    this.refreshLayerState();
  }
  ...
  getStructureBounds() {
    ...
    this.resources
      .filter((resource) => resource.name?.startsWith("solar_layer_") || resource.name === "solar_substrate")
      .forEach((resource) => bounds.expandByObject(resource));
    return bounds;
  }
